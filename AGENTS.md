...
- Keep core apps in the compile-time catalogue/loader path under `src/apps/`. Refresh owned public-repository apps with `pnpm sync:repositories` (authenticated `gh-axi`), then review the generated inventory and resulting launch targets; external apps must use credential-free HTTPS links—no runtime repository scanning, remote modules, or unreviewed iframes.
