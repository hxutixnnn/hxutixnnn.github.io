...
export function Dock({ apps, windowState, onActivate, surfaceRef, primaryTargetRef }: DockProps) {
  return (
    <nav
      ref={surfaceRef}
      aria-label="Dock"
    ...
    >
      {apps.map((app, index) => {
        const isOpen = index === 0 && windowState.presence === "open";
        const isMinimized = index === 0 && windowState.visibility === "minimized";
