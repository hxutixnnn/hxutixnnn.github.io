...
export function Dock({ apps, windowStates, onActivate, surfaceRef, targetRef }: DockProps) {
  return (
    <nav
      ...
      aria-label="Dock"
      data-dock-surface
      className="dock-surface fixed bottom-[max(10px,var(--tienos-safe-area-bottom))] left-1/2 z-[45] flex -translate-x-1/2 items-center rounded-[20px] border border-white/30 [background:linear-gradient(145deg,rgb(255_255_255/0.24),rgb(255_255_255/0.08)_42%,rgb(8_15_26/0.25)),var(--tienos-color-dock)] p-[7px] shadow-[0_18px_45px_rgb(0_0_0/0.4),0_3px_10px_rgb(0_0_0/0.24),inset_0_1px_0_rgb(255_255_255/0.48),inset_0_-1px_0_rgb(0_0_0/0.2)] backdrop-blur-[var(--tienos-blur-dock)] backdrop-saturate-[var(--tienos-saturate-dock)] contrast-more:border-2 contrast-more:border-[var(--tienos-color-border)] contrast-more:bg-[var(--tienos-color-dock)] [@media(prefers-reduced-transparency:reduce)]:bg-[var(--tienos-color-dock)] [@media(prefers-reduced-transparency:reduce)]:backdrop-filter-none [@media(forced-colors:active)]:border-[CanvasText] [@media(forced-colors:active)]:bg-[Canvas] [@media(forced-colors:active)]:shadow-none [@media(forced-colors:active)]:backdrop-filter-none"
    >
      {apps.map((app) => {
          ...
          <div key={app.id}>
            ...
            >
              {app.iconText ? (
                <span
                  aria-hidden="true"
                  className="grid size-9 place-items-center rounded-lg bg-[#20242a] text-[12px] font-semibold tracking-[-.08em] text-white shadow-inner"
                >
                  {app.iconText}
                </span>
              ) : (
