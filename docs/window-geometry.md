...
- `WindowFrame` consumes immutable workspace/frame values and each app's minimum dimensions, and is the only `react-rnd` owner.

## Invariants

For a usable workspace, clamped frames are finite and non-negative, honor the registered app's minimum dimensions when the workspace permits, remain below the viewport's right and bottom edges, start at or below the measured Menu Bar bottom, and end at or above the measured Dock/safe-area boundary. Repeated clamping is idempotent. Compact defaults remain tied to the current viewport/menu/Dock measurements; desktop restoration clamps the saved normal frame to the latest workspace with the app's minimum dimensions; compact restoration recomputes the compact default. Top/left resize policies retain the opposite edge whenever the requested size and usable bounds make that feasible.
