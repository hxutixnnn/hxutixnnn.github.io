...
Traffic lights retain 13px desktop and 11px compact dots with 20px center spacing, producing restrained 7px and 9px visible gaps. Three overlapping 44px-square semantic buttons preserve names, tab order, and coarse-pointer height; invisible child hit surfaces partition the 84px-wide cluster at the dot midpoints into exclusive 30px, 20px, and 34px regions. The narrower horizontal ownership is the necessary web tradeoff for familiar visual spacing and deterministic close, minimize, and fullscreen routing. Every point from the cluster's left through right edge belongs to exactly one control, focus renders around the dot outside the hit surface without clipping, and the complete cluster remains above the splitter and inside the compact window.
