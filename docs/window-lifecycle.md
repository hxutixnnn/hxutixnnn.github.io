...
The current registry contains retained System Settings and Calculator instances. The shell lifecycle boundary is keyed by `AppId`, so each registered app receives an independent single-window controller.
