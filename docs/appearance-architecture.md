...
Storage, current system theme and its subscription, wallpaper loader/decoder, document compositor, animation eligibility, decoded cache, and result observer are injected. The decoded cache is per service unless an explicit cache is supplied. Tests can therefore control every asynchronous boundary and create independent instances without resetting module state. Storage failure preserves the in-session commit.
...
The document compositor applies the root appearance/theme datasets, `colorScheme`, wallpaper fallback marker/property, and theme-color metadata as one commit inside the existing document-wide transition. Before desktop readiness, and when the compositor detects a hidden document, reduced motion/transparency, increased contrast, forced colors, or no rendered desktop, it commits directly. Otherwise native View Transition or the inert DOM snapshot fallback composes the whole document, including portals, Dock, and Settings. The fallback remains responsible for preserving rendered styles, form values, and scroll state.

## Inline parity boundary

The inline bootstrap must execute before module evaluation and therefore cannot import runtime modules. `validate:appearance-bootstrap` reads exported runtime definitions structurally, executes the human-readable inline bootstrap across valid, invalid, missing, malformed, Light, and Dark inputs, and verifies mode resolution, storage key behavior, media query, colors, preload wallpaper, and static wallpaper fallbacks. It is validation-only: source `index.html`, no-JS behavior, startup failure release, and timeout remain independently useful.

## Phase 6 validation evidence

| Measure                                      |                                                                             Before |                                                                        After |
| -------------------------------------------- | ---------------------------------------------------------------------------------: | ---------------------------------------------------------------------------: |
| Zustand adapter                              | 169 lines; persistence, media, image decode, DOM commit, transaction counter/cache |                83 lines; service snapshot projection and stable actions only |
