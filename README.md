...
Playwright coverage is split by regression boundary in `tests/e2e/`: `startup`, `menus`, `settings-layout`, `calculator`, `appearance`, `window-lifecycle`, `spotlight`, and `notes`. Put a regression in the narrowest owning contract; lifecycle/geometry/input behavior belongs in `window-lifecycle`, Calculator behavior belongs in `calculator`, Spotlight search and shell-overlay behavior belongs in `spotlight`, Settings content and control layout belongs in `settings-layout`, and Notes behavior belongs in `notes`. Small semantic interactions live in `tests/e2e/drivers/` (`desktop`, `settingsWindow`, and `appearance`); keep direct assertions in specs rather than growing a page-object framework. The exhaustive reducer transition table is in `src/windows/singleWindowMachine.test.ts`; React-boundary characterization remains in `tests/lifecycle-characterization.test.ts`. The lifecycle architecture and invariants are owned by `docs/window-lifecycle.md`; Settings app and pane ownership is documented in [`docs/system-settings.md`](docs/system-settings.md).
